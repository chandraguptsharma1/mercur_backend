...
export default async function seedMarketplaceData({ container }: ExecArgs) {
  ...
  // === ALGOLIA SYNC ===
  logger.info('Syncing products and reviews to Algolia...')
  const syncExistingProductsWithAlgolia = (
    await import('./scripts/sync-algolia')
  ).default
  await syncExistingProductsWithAlgolia({ container })
